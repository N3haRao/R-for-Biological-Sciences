...
summarize_rows <- function(x, fn, na.rm=FALSE) {
  if (na.rm) {
    x <- apply(x, 1, function(row) fn(row, na.rm = TRUE))
  } else {
    x <- apply(x, 1, fn)
  }
  return(x)
...
summarize_matrix <- function(x, na.rm = FALSE) {
  if (na.rm) {
    x_mean <- apply(x, 1, mean, na.rm = TRUE)
    x_stdev <- apply(x, 1, sd, na.rm = TRUE)
    x_median <- apply(x, 1, median, na.rm = TRUE)
    x_min <- apply(x, 1, min, na.rm = TRUE)
    x_max <- apply(x, 1, max, na.rm = TRUE)
    x_num_lt_0 <- apply(x, 1, function(row) sum(row < 0, na.rm = TRUE))
    x_num_btw_1_and_5 <- apply(x, 1, function(row) sum(is_between(row, 1, 5), na.rm = TRUE))
    x_num_na <- apply(x, 1, function(row) sum(is.na(row)))
  } else {
    x_mean <- apply(x, 1, mean)
    x_stdev <- apply(x, 1, sd)
    x_median <- apply(x, 1, median)
    x_min <- apply(x, 1, min)
    x_max <- apply(x, 1, max)
    x_num_lt_0 <- apply(x, 1, function(row) sum(row < 0))
    x_num_btw_1_and_5 <- apply(x, 1, function(row) sum(is_between(row, 1, 5)))
    x_num_na <- apply(x, 1, function(row) sum(is.na(row)))
  }
  result_df <- data.frame(
    ...
    max = x_max,
    num_lt_0 = x_num_lt_0,
    num_btw_1_and_5 = x_num_btw_1_and_5,
    num_na = x_num_na
  )
  return(result_df)
}
#Now, you have filled in the function bodies in the main.R file based on the descriptions and examples provided in the report. You can test these functions by executing the code chunks in the report.Rmd file and compare the output to the expected results.
